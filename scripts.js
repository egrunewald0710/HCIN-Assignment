...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (browseContainer) {
    // ✅ Keep hardcoded notes already in HTML
    // So we don’t clear browseContainer.innerHTML — we just add uploaded notes below
  ...
  if (myNotesContainer) {
    ...
    if (notes.length > 0) {
      ...
      // Show message only if there are no uploaded notes
      if (!myNotesContainer.querySelector(".note-card")) {
